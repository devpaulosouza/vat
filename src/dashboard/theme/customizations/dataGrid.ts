...
export const dataGridCustomizations: DataGridProComponents<Theme> & DataGridComponents<Theme> = {
  MuiDataGrid: {
    styleOverrides: {
      ...
      cell: ({ theme }: ThemeProps) => ({ borderTopColor: (theme.vars || theme).palette.divider }),
      menu: ({ theme }: ThemeProps) => ({
      ...
      iconButtonContainer: ({ theme }: ThemeProps) => ({
      ...
      menuIconButton: ({ theme }: ThemeProps) => ({
        border: 'none',
        backgroundColor: 'transparent',
        '&:hover': {
      ...
      filterForm: ({ theme }: ThemeProps) => ({
        gap: theme.spacing(1),
        alignItems: 'flex-end',
      }),
      columnsManagementHeader: ({ theme }: ThemeProps) => ({
